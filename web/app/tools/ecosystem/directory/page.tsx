...
function ScoreBadge({ score }: { score: number }) {
    ...
    else if (score >= 40) color = 'text-[#FFF] border-[#FFF]/50 bg-[#FFF]/10'; // brand/warning equivalent
    ...
    return (
        <div className="glass-card hover-glow p-6 md:p-8 rounded-3xl border border-white/5 h-full flex flex-col relative group transition-all duration-300 bg-[#050505] hover:bg-white/5">
            ...
            {open && (
                <div className="mb-8 pt-4 border-t border-white/10 animate-fade-in">
                    <ul className="space-y-3">
                        ...
                        <li className="text-sm text-text-secondary font-light flex justify-between gap-4 border-b border-white/5 pb-2">
                            <span className="shrink-0 text-white font-medium">Capacity (Conf/Booths):</span>
                            <span className="text-right">{item.confHallCapacity} / {item.callBooths}</span>
                        </li>
                    </ul>
                </div>
            )}
            ...
            <div className="mt-auto flex flex-col gap-3">
                ...
                {item.googleMapsLink && (
                    <a href={item.googleMapsLink} target="_blank" rel="noopener noreferrer" className="w-full bg-transparent border border-white/20 hover:border-white text-white py-3 rounded-full font-bold transition duration-300 flex justify-center items-center gap-2 text-sm">
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                ...
                <div className="text-center mb-16 max-w-3xl mx-auto border-b border-white/10 pb-12">
                    ...
                    <p className="text-lg md:text-xl text-text-secondary font-light">
                        Mapping over 50 top coworking spaces and incubators. Compare fees, read equity guarantees, and access mentors directly.
                    </p>
                </div>
                ...
                {showFilters && (
                    <div className="glass-card p-6 md:p-8 rounded-3xl border border-white/10 mb-12 bg-[#050505]">
                        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
                            ...
                            <div className="flex flex-col gap-3">
                                <label className="flex items-center gap-2 text-xs font-bold text-text-secondary tracking-widest uppercase"><Building2 size={14} className="text-accent-blue" /> Corp Type</label>
                                <div className="flex flex-wrap gap-2">
                                    {broadTypes.map(bt => (
                                        <button key={bt} onClick={() => setBroadTypeFilter(bt)} className={`px-3 py-1.5 text-xs rounded-full border transition-colors ${broadTypeFilter === bt ? 'bg-white text-black border-white font-bold' : 'bg-transparent text-text-secondary border-white/20 hover:border-white/50'}`}>
                                            {bt}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="flex flex-col gap-3">
                                <label className="flex items-center gap-2 text-xs font-bold text-text-secondary tracking-widest uppercase"><TrendingUp size={14} className="text-accent-blue" /> Equity Models</label>
                                <div className="flex flex-wrap gap-2">
                                    {equityCategories.map(ec => (
                                        <button key={ec} onClick={() => setEquityCategoryFilter(ec)} className={`px-3 py-1.5 text-xs rounded-full border transition-colors ${equityCategoryFilter === ec ? 'bg-white text-black border-white font-bold' : 'bg-transparent text-text-secondary border-white/20 hover:border-white/50'}`}>
                                            {ec}
                                        </button>
                                    ))}
                                </div>
                        ...
                        <div className="pt-6 border-t border-white/10 flex justify-center">
                            <button className="text-sm text-text-secondary hover:text-white transition-colors" onClick={resetFilters}>
                                Execute Reset
                            </button>
                        </div>
                    </div>
                ...
                {filteredData.length > 0 ? (
                    ...
                    <div className="glass-card p-16 rounded-3xl border border-dashed border-white/10 text-center flex flex-col items-center justify-center min-h-[400px]">
                        ...
                        <p className="text-text-secondary mb-8">No incubators or spaces matched this specific query setup.</p>
